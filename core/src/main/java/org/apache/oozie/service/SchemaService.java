...
public class SchemaService implements Service {
	...
	public static final String WF_CONF_EXT_SCHEMAS = CONF_PREFIX
			+ "wf.ext.schemas";

	public static final String COORD_CONF_EXT_SCHEMAS = CONF_PREFIX
			+ "coord.ext.schemas";

	public static final String BUNDLE_CONF_EXT_SCHEMAS = CONF_PREFIX
			+ "bundle.ext.schemas";

	public static final String SLA_CONF_EXT_SCHEMAS = CONF_PREFIX
			+ "sla.ext.schemas";
	...
	private static final String OOZIE_WORKFLOW_XSD[] = {
			"oozie-workflow-0.1.xsd", "oozie-workflow-0.2.xsd",
			"oozie-workflow-0.2.5.xsd", "oozie-workflow-0.3.xsd",
			"oozie-workflow-0.4.xsd" };
	private static final String OOZIE_COORDINATOR_XSD[] = {
			"oozie-coordinator-0.1.xsd", "oozie-coordinator-0.2.xsd",
	...
	private Schema loadSchema(Configuration conf, String[] baseSchemas,
			String extSchema) throws SAXException, IOException {
		List<StreamSource> sources = new ArrayList<StreamSource>();
		for (String baseSchema : baseSchemas) {
			sources.add(new StreamSource(IOUtils.getResourceAsStream(
					baseSchema, -1)));
		}
		...
		return factory.newSchema(sources.toArray(new StreamSource[sources
				.size()]));
	}
	...
	public void init(Services services) throws ServiceException {
		try {
			wfSchema = loadSchema(services.getConf(), OOZIE_WORKFLOW_XSD,
					WF_CONF_EXT_SCHEMAS);
			coordSchema = loadSchema(services.getConf(), OOZIE_COORDINATOR_XSD,
	 ...
	 * 
	 * @param schemaName
	 *            : Name of schema definition (i.e. WORKFLOW/COORDINATOR/BUNDLE)
	 * @return the schema for XML validation of application definitions.
	 */
	public Schema getSchema(SchemaName schemaName) {
		...
		if (schemaName == SchemaName.WORKFLOW) {
			...
			throw new RuntimeException("No schema found with name "
					+ schemaName);
		}
		return returnSchema;
	}
